...
```
### Lambda　Function URL
`https://dqqvl5huzxx3uy7vih5xn5elly0poazt.lambda-url.ap-northeast-1.on.aws/`

### パス構成
- `/redirect/?id=<encrypt_id>` ※URLエンコード必須
  - encrypt_idはURLエンコード必須
  - Lambda関数でencrypt_idがデコードされ、対応したwork_idのリダイレクト先に飛ぶ
- `/test`
  - google.comへリダイレクトされるという表示がされる
